<?php
require_once 'config.php';        // Per session_start(), BASE_URL, ecc.
require_once 'auth_check.php';   // Per is_logged_in() e require_admin()
require_admin();                 // QUESTA È LA CHIAMATA FONDAMENTALE PER LA PROTEZIONE ADMIN
require_once 'utils/functions.php'; // Per display_flash_message() e altre utility

// Recupera il nome dell'admin per la visualizzazione, se necessario
$admin_name = $_SESSION['user_name'] ?? 'Admin';
$current_tab = $_GET['tab'] ?? 'overview'; // Default to overview
?>
<!DOCTYPE html>
<html lang="it">
  <head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <link rel="preconnect" href="https://fonts.gstatic.com/" crossorigin="" />
    <link
      rel="stylesheet"
      as="style"
      onload="this.rel='stylesheet'"
      href="https://fonts.googleapis.com/css2?display=swap&amp;family=Noto+Sans%3Awght%40400%3B500%3B700%3B900&amp;family=Space+Grotesk%3Awght%40400%3B500%3B700"
    />
    <title>Admin Dashboard - CN Auto</title>
    <link rel="icon" type="image/x-icon" href="data:image/x-icon;base64," />
    <script src="https://cdn.tailwindcss.com?plugins=forms,container-queries"></script>
  </head>
  <body>
    <div class="relative flex size-full min-h-screen flex-col bg-[#232010] dark group/design-root overflow-x-hidden" style='font-family: "Space Grotesk", "Noto Sans", sans-serif;'>
      <div class="layout-container flex h-full grow flex-col">
        <div class="flex flex-col sm:flex-row gap-1 px-2 sm:px-6 flex-1 justify-center py-5">
          <!-- Sidebar -->
          <div class="layout-content-container flex flex-col w-full sm:w-64 md:w-80 bg-[#2c281a] p-4 rounded-lg sm:min-h-[700px] mb-4 sm:mb-0 sm:mr-1 self-start">
            <div class="flex h-full flex-col justify-between">
              <div class="flex flex-col gap-4">
                <div class="flex flex-col mb-4">
                  <a href="<?php echo BASE_URL . '/dashboardAdmin.php'; ?>" class="text-white text-xl font-bold leading-normal hover:text-[#fcdd53]">CN Workshop</a>
                  <p class="text-[#cdc28e] text-sm font-normal leading-normal"><?php echo htmlspecialchars($admin_name); ?> Panel</p>
                </div>
                <nav class="flex flex-col gap-2">
                  <a href="<?php echo BASE_URL . '/dashboardAdmin.php?tab=overview'; ?>" class="flex items-center gap-3 px-3 py-2 rounded-lg hover:bg-[#4a4321] text-white <?php echo ($current_tab === 'overview') ? 'bg-[#4a4321]' : ''; ?>">
                    <div class="text-white" data-icon="House" data-size="24px" data-weight="fill">
                      <svg xmlns="http://www.w3.org/2000/svg" width="24px" height="24px" fill="currentColor" viewBox="0 0 256 256"><path d="M224,115.55V208a16,16,0,0,1-16,16H168a16,16,0,0,1-16-16V168a8,8,0,0,0-8-8H112a8,8,0,0,0-8,8v40a16,16,0,0,1-16,16H48a16,16,0,0,1-16-16V115.55a16,16,0,0,1,5.17-11.78l80-75.48.11-.11a16,16,0,0,1,21.53,0,1.14,1.14,0,0,0,.11.11l80,75.48A16,16,0,0,1,224,115.55Z"></path></svg>
                    </div>
                    <p class="text-sm font-medium leading-normal">Dashboard</p>
                  </a>
                  <a href="<?php echo BASE_URL . '/dashboardAdmin.php?tab=appointments'; ?>" class="flex items-center gap-3 px-3 py-2 rounded-lg hover:bg-[#4a4321] text-white <?php echo ($current_tab === 'appointments') ? 'bg-[#4a4321]' : ''; ?>">
                    <div class="text-white" data-icon="Calendar" data-size="24px" data-weight="regular">
                      <svg xmlns="http://www.w3.org/2000/svg" width="24px" height="24px" fill="currentColor" viewBox="0 0 256 256"><path d="M208,32H184V24a8,8,0,0,0-16,0v8H88V24a8,8,0,0,0-16,0v8H48A16,16,0,0,0,32,48V208a16,16,0,0,0,16,16H208a16,16,0,0,0,16-16V48A16,16,0,0,0,208,32ZM72,48v8a8,8,0,0,0,16,0V48h80v8a8,8,0,0,0,16,0V48h24V80H48V48ZM208,208H48V96H208V208Zm-96-88v64a8,8,0,0,1-16,0V132.94l-4.42,2.22a8,8,0,0,1-7.16-14.32l16-8A8,8,0,0,1,112,120Zm59.16,30.45L152,176h16a8,8,0,0,1,0,16H136a8,8,0,0,1-6.4-12.8l28.78-38.37A8,8,0,1,0,145.07,132a8,8,0,1,1-13.85-8A24,24,0,0,1,176,136,23.76,23.76,0,0,1,171.16,150.45Z"></path></svg>
                    </div>
                    <p class="text-sm font-medium leading-normal">Appointments</p>
                  </a>
                   <a href="<?php echo BASE_URL . '/dashboardAdmin.php?tab=add_booking'; ?>" class="flex items-center gap-3 px-3 py-2 rounded-lg hover:bg-[#4a4321] text-white <?php echo ($current_tab === 'add_booking') ? 'bg-[#4a4321]' : ''; ?>">
                    <div class="text-white" data-icon="CalendarPlus" data-size="24px" data-weight="regular">
                        <svg xmlns="http://www.w3.org/2000/svg" width="24px" height="24px" fill="currentColor" viewBox="0 0 256 256"><path d="M208,32H184V24a8,8,0,0,0-16,0v8H88V24a8,8,0,0,0-16,0v8H48A16,16,0,0,0,32,48V208a16,16,0,0,0,16,16H208a16,16,0,0,0,16-16V48A16,16,0,0,0,208,32ZM72,48v8a8,8,0,0,0,16,0V48h80v8a8,8,0,0,0,16,0V48h24V80H48V48ZM208,208H48V96H208V208Zm-40-80h-16v16a8,8,0,0,1-16,0V128H120a8,8,0,0,1,0-16h16V96a8,8,0,0,1,16,0v16h16a8,8,0,0,1,0,16Z"></path></svg>
                    </div>
                    <p class="text-sm font-medium leading-normal">Add Booking</p>
                  </a>
                  <a href="<?php echo BASE_URL . '/dashboardAdmin.php?tab=services'; ?>" class="flex items-center gap-3 px-3 py-2 rounded-lg hover:bg-[#4a4321] text-white <?php echo ($current_tab === 'services') ? 'bg-[#4a4321]' : ''; ?>">
                    <div class="text-white" data-icon="Gear" data-size="24px" data-weight="regular">
                      <svg xmlns="http://www.w3.org/2000/svg" width="24px" height="24px" fill="currentColor" viewBox="0 0 256 256"><path d="M128,80a48,48,0,1,0,48,48A48.05,48.05,0,0,0,128,80Zm0,80a32,32,0,1,1,32-32A32,32,0,0,1,128,160Zm88-29.84q.06-2.16,0-4.32l14.92-18.64a8,8,0,0,0,1.48-7.06,107.21,107.21,0,0,0-10.88-26.25,8,8,0,0,0-6-3.93l-23.72-2.64q-1.48-1.56-3-3L186,40.54a8,8,0,0,0-3.94-6,107.71,107.71,0,0,0-26.25-10.87,8,8,0,0,0-7.06,1.49L130.16,40Q128,40,125.84,40L107.2,25.11a8,8,0,0,0-7.06-1.48A107.6,107.6,0,0,0,73.89,34.51a8,8,0,0,0-3.93,6L67.32,64.27q-1.56,1.49-3,3L40.54,70a8,8,0,0,0-6,3.94,107.71,107.71,0,0,0-10.87,26.25,8,8,0,0,0,1.49,7.06L40,125.84Q40,128,40,130.16L25.11,148.8a8,8,0,0,0-1.48,7.06,107.21,107.21,0,0,0,10.88,26.25,8,8,0,0,0,6,3.93l23.72,2.64q1.49,1.56,3,3L70,215.46a8,8,0,0,0,3.94,6,107.71,107.71,0,0,0,26.25,10.87,8,8,0,0,0,7.06-1.49L125.84,216q2.16.06,4.32,0l18.64,14.92a8,8,0,0,0,7.06,1.48,107.21,107.21,0,0,0,26.25-10.88,8,8,0,0,0,3.93-6l2.64-23.72q1.56-1.48,3-3L215.46,186a8,8,0,0,0,6-3.94,107.71,107.71,0,0,0,10.87-26.25,8,8,0,0,0-1.49-7.06Zm-16.1-6.5a73.93,73.93,0,0,1,0,8.68,8,8,0,0,0,1.74,5.48l14.19,17.73a91.57,91.57,0,0,1-6.23,15L187,173.11a8,8,0,0,0-5.1,2.64,74.11,74.11,0,0,1-6.14,6.14,8,8,0,0,0-2.64,5.1l-2.51,22.58a91.32,91.32,0,0,1-15,6.23l-17.74-14.19a8,8,0,0,0-5-1.75h-.48a73.93,73.93,0,0,1-8.68,0,8,8,0,0,0-5.48,1.74L100.45,215.8a91.57,91.57,0,0,1-15-6.23L82.89,187a8,8,0,0,0-2.64-5.1,74.11,74.11,0,0,1-6.14-6.14,8,8,0,0,0-5.1-2.64L46.43,170.6a91.32,91.32,0,0,1-6.23-15l14.19-17.74a8,8,0,0,0,1.74-5.48,73.93,73.93,0,0,1,0-8.68,8,8,0,0,0-1.74-5.48L40.2,100.45a91.57,91.57,0,0,1,6.23-15L69,82.89a8,8,0,0,0,5.1-2.64,74.11,74.11,0,0,1,6.14-6.14A8,8,0,0,0,82.89,69L85.4,46.43a91.32,91.32,0,0,1,15-6.23l17.74,14.19a8,8,0,0,0,5.48,1.74,73.93,73.93,0,0,1,8.68,0,8,8,0,0,0,5.48-1.74L155.55,40.2a91.57,91.57,0,0,1,15,6.23L173.11,69a8,8,0,0,0,2.64,5.1,74.11,74.11,0,0,1,6.14,6.14,8,8,0,0,0,5.1,2.64l22.58,2.51a91.32,91.32,0,0,1,6.23,15l-14.19,17.74A8,8,0,0,0,199.87,123.66Z"></path></svg>
                    </div>
                    <p class="text-sm font-medium leading-normal">Services</p>
                  </a>
                  <a href="<?php echo BASE_URL . '/dashboardAdmin.php?tab=users'; ?>" class="flex items-center gap-3 px-3 py-2 rounded-lg hover:bg-[#4a4321] text-white <?php echo ($current_tab === 'users') ? 'bg-[#4a4321]' : ''; ?>">
                    <div class="text-white" data-icon="Users" data-size="24px" data-weight="regular">
                      <svg xmlns="http://www.w3.org/2000/svg" width="24px" height="24px" fill="currentColor" viewBox="0 0 256 256"><path d="M117.25,157.92a60,60,0,1,0-66.5,0A95.83,95.83,0,0,0,3.53,195.63a8,8,0,1,0,13.4,8.74,80,80,0,0,1,134.14,0,8,8,0,0,0,13.4-8.74A95.83,95.83,0,0,0,117.25,157.92ZM40,108a44,44,0,1,1,44,44A44.05,44.05,0,0,1,40,108Zm210.14,98.7a8,8,0,0,1-11.07-2.33A79.83,79.83,0,0,0,172,168a8,8,0,0,1,0-16,44,44,0,1,0-16.34-84.87,8,8,0,1,1-5.94-14.85,60,60,0,0,1,55.53,105.64,95.83,95.83,0,0,1,47.22,37.71A8,8,0,0,1,250.14,206.7Z"></path></svg>
                    </div>
                    <p class="text-sm font-medium leading-normal">Users</p>
                  </a>
                  <!-- Staff and Availability are static for now -->
                  <div class="flex items-center gap-3 px-3 py-2 text-gray-500 cursor-not-allowed opacity-50">
                    <div class="text-gray-500" data-icon="UsersThree" data-size="24px" data-weight="regular">
                       <svg xmlns="http://www.w3.org/2000/svg" width="24px" height="24px" fill="currentColor" viewBox="0 0 256 256"><path d="M117.25,157.92a60,60,0,1,0-66.5,0A95.83,95.83,0,0,0,3.53,195.63a8,8,0,1,0,13.4,8.74,80,80,0,0,1,134.14,0,8,8,0,0,0,13.4-8.74A95.83,95.83,0,0,0,117.25,157.92ZM40,108a44,44,0,1,1,44,44A44.05,44.05,0,0,1,40,108Zm210.14,98.7a8,8,0,0,1-11.07-2.33A79.83,79.83,0,0,0,172,168a8,8,0,0,1,0-16,44,44,0,1,0-16.34-84.87,8,8,0,1,1-5.94-14.85,60,60,0,0,1,55.53,105.64,95.83,95.83,0,0,1,47.22,37.71A8,8,0,0,1,250.14,206.7Z"></path></svg>
                    </div>
                    <p class="text-sm font-medium leading-normal">Staff (Soon)</p>
                  </div>
                  <div class="flex items-center gap-3 px-3 py-2 text-gray-500 cursor-not-allowed opacity-50">
                    <div class="text-gray-500" data-icon="Clock" data-size="24px" data-weight="regular">
                      <svg xmlns="http://www.w3.org/2000/svg" width="24px" height="24px" fill="currentColor" viewBox="0 0 256 256"><path d="M128,24A104,104,0,1,0,232,128,104.11,104.11,0,0,0,128,24Zm0,192a88,88,0,1,1,88-88A88.1,88.1,0,0,1,128,216Zm64-88a8,8,0,0,1-8,8H128a8,8,0,0,1-8-8V72a8,8,0,0,1,16,0v48h48A8,8,0,0,1,192,128Z"></path></svg>
                    </div>
                    <p class="text-sm font-medium leading-normal">Availability (Soon)</p>
                  </div>
                </nav>
              </div>
              <div class="flex flex-col gap-1 mt-auto pt-4 border-t border-[#4a4321]">
                <a href="<?php echo BASE_URL . '/home.php'; ?>" target="_blank" class="flex items-center gap-3 px-3 py-2 rounded-lg hover:bg-[#4a4321] text-white">
                    <div class="text-white" data-icon="Globe" data-size="24px" data-weight="regular">
                        <svg xmlns="http://www.w3.org/2000/svg" width="24px" height="24px" fill="currentColor" viewBox="0 0 256 256"><path d="M128,24a104,104,0,1,0,104,104A104.11,104.11,0,0,0,128,24Zm0,192a88,88,0,1,1,88-88A88.1,88.1,0,0,1,128,216Zm80-88a80.12,80.12,0,0,1-25.79,58.62L100.6,72.21A80.12,80.12,0,0,1,208,128ZM48,128A79.57,79.57,0,0,1,72.21,70L183.8,187.79A79.66,79.66,0,0,1,128,208h0A79.57,79.57,0,0,1,48,128Z"></path></svg>
                    </div>
                    <p class="text-sm font-medium leading-normal">View Public Site</p>
                </a>
                <a href="<?php echo BASE_URL . '/dashboardAdmin.php?tab=settings'; ?>" class="flex items-center gap-3 px-3 py-2 rounded-lg hover:bg-[#4a4321] text-white <?php echo ($current_tab === 'settings') ? 'bg-[#4a4321]' : ''; ?>">
                  <div class="text-white" data-icon="GearSix" data-size="24px" data-weight="regular">
                    <svg xmlns="http://www.w3.org/2000/svg" width="24px" height="24px" fill="currentColor" viewBox="0 0 256 256"><path d="M128,80a48,48,0,1,0,48,48A48.05,48.05,0,0,0,128,80Zm0,80a32,32,0,1,1,32-32A32,32,0,0,1,128,160Zm88-29.84q.06-2.16,0-4.32l14.92-18.64a8,8,0,0,0,1.48-7.06,107.21,107.21,0,0,0-10.88-26.25,8,8,0,0,0-6-3.93l-23.72-2.64q-1.48-1.56-3-3L186,40.54a8,8,0,0,0-3.94-6,107.71,107.71,0,0,0-26.25-10.87,8,8,0,0,0-7.06,1.49L130.16,40Q128,40,125.84,40L107.2,25.11a8,8,0,0,0-7.06-1.48A107.6,107.6,0,0,0,73.89,34.51a8,8,0,0,0-3.93,6L67.32,64.27q-1.56,1.49-3,3L40.54,70a8,8,0,0,0-6,3.94,107.71,107.71,0,0,0-10.87,26.25,8,8,0,0,0,1.49,7.06L40,125.84Q40,128,40,130.16L25.11,148.8a8,8,0,0,0-1.48,7.06,107.21,107.21,0,0,0,10.88,26.25,8,8,0,0,0,6,3.93l23.72,2.64q1.49,1.56,3,3L70,215.46a8,8,0,0,0,3.94,6,107.71,107.71,0,0,0,26.25,10.87,8,8,0,0,0,7.06-1.49L125.84,216q2.16.06,4.32,0l18.64,14.92a8,8,0,0,0,7.06,1.48,107.21,107.21,0,0,0,26.25-10.88,8,8,0,0,0,3.93-6l2.64-23.72q1.56-1.48,3-3L215.46,186a8,8,0,0,0,6-3.94,107.71,107.71,0,0,0,10.87-26.25,8,8,0,0,0-1.49-7.06Zm-16.1-6.5a73.93,73.93,0,0,1,0,8.68,8,8,0,0,0,1.74,5.48l14.19,17.73a91.57,91.57,0,0,1-6.23,15L187,173.11a8,8,0,0,0-5.1,2.64,74.11,74.11,0,0,1-6.14,6.14,8,8,0,0,0-2.64,5.1l-2.51,22.58a91.32,91.32,0,0,1-15,6.23l-17.74-14.19a8,8,0,0,0-5-1.75h-.48a73.93,73.93,0,0,1-8.68,0,8,8,0,0,0-5.48,1.74L100.45,215.8a91.57,91.57,0,0,1-15-6.23L82.89,187a8,8,0,0,0-2.64-5.1,74.11,74.11,0,0,1-6.14-6.14,8,8,0,0,0-5.1-2.64L46.43,170.6a91.32,91.32,0,0,1-6.23-15l14.19-17.74a8,8,0,0,0,1.74-5.48,73.93,73.93,0,0,1,0-8.68,8,8,0,0,0-1.74-5.48L40.2,100.45a91.57,91.57,0,0,1,6.23-15L69,82.89a8,8,0,0,0,5.1-2.64,74.11,74.11,0,0,1,6.14-6.14A8,8,0,0,0,82.89,69L85.4,46.43a91.32,91.32,0,0,1,15-6.23l17.74,14.19a8,8,0,0,0,5.48,1.74,73.93,73.93,0,0,1,8.68,0,8,8,0,0,0,5.48-1.74L155.55,40.2a91.57,91.57,0,0,1,15,6.23L173.11,69a8,8,0,0,0,2.64,5.1,74.11,74.11,0,0,1,6.14,6.14,8,8,0,0,0,5.1,2.64l22.58,2.51a91.32,91.32,0,0,1,6.23,15l-14.19,17.74A8,8,0,0,0,199.87,123.66Z"></path></svg>
                  </div>
                  <p class="text-sm font-medium leading-normal">Settings</p>
                </a>
                 <a href="<?php echo BASE_URL . '/logout.php'; ?>" class="flex items-center gap-3 px-3 py-2 rounded-lg hover:bg-[#4a4321] text-white">
                    <div class="text-white" data-icon="SignOut" data-size="24px" data-weight="regular">
                        <svg xmlns="http://www.w3.org/2000/svg" width="24px" height="24px" fill="currentColor" viewBox="0 0 256 256"><path d="M112,216a8,8,0,0,1-8,8H48a16,16,0,0,1-16-16V48A16,16,0,0,1,48,32h56a8,8,0,0,1,0,16H48V208h56A8,8,0,0,1,112,216Zm109.66-93.66-48-48a8,8,0,0,0-11.32,11.32L196.69,120H104a8,8,0,0,0,0,16h92.69l-34.35,34.34a8,8,0,0,0,11.32,11.32l48-48A8,8,0,0,0,221.66,122.34Z"></path></svg>
                    </div>
                    <p class="text-sm font-medium leading-normal">Logout</p>
                </a>
              </div>
            </div>
          </div>
          <!-- Main Content Area -->
          <div class="layout-content-container flex flex-col flex-1 max-w-full sm:max-w-[calc(100%-16rem-0.25rem)] md:max-w-[calc(100%-20rem-0.25rem)] bg-[#2c281a] p-1 sm:p-0 rounded-lg">
             <?php echo display_flash_message(); ?>
            <?php
                switch ($current_tab) {
                    case 'appointments':
                        include __DIR__ . '/partials/admin_appointments.php';
                        break;
                    case 'add_booking':
                        include __DIR__ . '/partials/admin_add_booking_form.php';
                        break;
                    case 'services':
                        include __DIR__ . '/partials/admin_services.php';
                        break;
                    case 'users':
                        include __DIR__ . '/partials/admin_users.php';
                        break;
                    case 'settings':
                         include __DIR__ . '/partials/admin_settings.php';
                        break;
                    case 'overview':
                    default:
                        include __DIR__ . '/partials/admin_overview.php';
                        break;
                }
            ?>
          </div>
        </div>
        <footer class="flex justify-center border-t border-solid border-t-[#4a4321] mt-auto py-5 bg-[#232010]">
            <div class="flex max-w-[960px] flex-1 flex-col px-4">
                <p class="text-[#cdc28e] text-xs sm:text-sm font-normal leading-normal text-center">© <?php echo date("Y"); ?> CN Auto Admin. All rights reserved.</p>
            </div>
        </footer>
      </div>
    </div>
  </body>
</html>
